...
import {
  ADMIN_PASSCODE, B, DATA_BRANCH, GITHUB_API_BASE, IDEA_STATUS_META,
} from '../constants'
import { CATALOG_FALLBACK } from '../data-fallback'
import { getGitHubPAT, setGitHubPAT, getCatalogSHA, setCatalogSHA } from '../storage'
import { Callout } from '../components/Callout'

const CATALOG_PATH = 'catalog.json'

async function saveCatalogToGitHub(catalog, expectedSha) {
  ...
  const body = {
    message: `admin: update catalog - ${timestamp}`,
    content,
    branch: DATA_BRANCH,
  }
  if (expectedSha) body.sha = expectedSha

  const res = await fetch(`${GITHUB_API_BASE}/contents/${CATALOG_PATH}`, {
    method: 'PUT',
    headers: {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) throw new Error('PAT expired or invalid. Please update your GitHub token.')
...
const PageAdmin = ({ liveTiles, onCatalogUpdate, liveIdeas, onIdeasUpdate }) => {
  const _liveIdeas = liveIdeas || CATALOG_FALLBACK.ideas
  ...
  const [patSaved, setPatSaved] = useState(() => !!getGitHubPAT())
  ...
  const handleIdeasApply = async () => {
    ...
    try {
      ...
      const sha = await saveCatalogToGitHub(catalog, getCatalogSHA())
      if (sha) setCatalogSHA(sha)
      onIdeasUpdate(ideasParsed)
      setIdeasSaveSuccess(true)
      setTimeout(() => setIdeasSaveSuccess(false), 3000)
    } catch (e) {
      setIdeasErr(e.message === 'CONFLICT'
        ? 'Data changed since you loaded. Refreshing...'
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const jsonSize = new Blob([JSON.stringify(catalog)]).size
      if (jsonSize > 500000) {
        setUploadErr('Catalog exceeds 500KB. Reduce data and try again.')
        setSaving(false)
        return
      }
      ...
      if (e.message === 'CONFLICT') {
        setUploadErr('Data changed since you loaded. Refreshing...')
        try {
          const res = await fetch(`${GITHUB_API_BASE}/contents/${CATALOG_PATH}?ref=${DATA_BRANCH}`, {
            headers: { Accept: 'application/vnd.github+json' },
          })
          if (res.ok) {
            const data = await res.json()
            setCatalogSHA(data.sha)
          }
        } catch { /* ignore */ }
      } else {
  ...
  // ── Unlocked state ────────────────────────────────────────────────────────────
  const isOverride = liveTiles !== CATALOG_FALLBACK.tiles
  ...
  return (
    <div>
      ...
      {/* PAT setup */}
      {!patSaved && (
        <div style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 10,
          padding: '16px 20px', marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#805700', marginBottom: 8 }}>GitHub PAT Required</div>
          <div style={{ fontSize: 12.5, color: B.muted, marginBottom: 12, lineHeight: 1.6 }}>
            Enter a fine-grained PAT with <code style={{ fontSize: 11.5, background: B.surface2, padding: '1px 5px', borderRadius: 3 }}>Contents: write</code> permission scoped to this repo only.
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input type='password' value={patInput} onChange={(e) => setPatInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSavePAT()}
              placeholder='ghp_xxxx or github_pat_xxxx'
              style={{ flex: 1, padding: '10px 12px', fontSize: 14, border: `1px solid ${B.border}`,
                borderRadius: 7, fontFamily: 'inherit', outline: 'none' }} />
      ...
      <div style={{ background: B.white, border: `1px solid ${B.border}`, borderRadius: 10, padding: '22px 24px', marginBottom: 20 }}>
        ...
        {diff && parsed && (
          <div style={{ marginTop: 16 }}>
            ...
            <button onClick={handleApply} disabled={saving || !patSaved}
              style={{ background: (saving || !patSaved) ? B.surface2 : B.blue, color: (saving || !patSaved) ? B.dim : '#fff',
                fontWeight: 700, fontSize: 14, padding: '11px 24px', borderRadius: 7,
                border: 'none', cursor: (saving || !patSaved) ? 'default' : 'pointer', marginTop: 4 }}>
              {saving ? 'Saving...' : `Apply ${parsed.length} Tiles`}
            </button>
            {!patSaved && <div style={{ fontSize: 11, color: B.dim, marginTop: 6 }}>Save a GitHub PAT above to enable publishing.</div>}
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: 28, paddingTop: 24, borderTop: `2px solid ${B.border}` }}>
        ...
        <div style={{ background: B.white, border: `1px solid ${B.border}`, borderRadius: 10,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
            {_liveIdeas !== CATALOG_FALLBACK.ideas && (
              <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 4,
                background: 'rgba(99,223,78,0.12)', color: '#1a6010', border: '1px solid rgba(99,223,78,0.3)' }}>
                Custom ideas active
              </span>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              {_liveIdeas !== CATALOG_FALLBACK.ideas && !ideasResetConfirm && (
        ...
        <div style={{ background: B.white, border: `1px solid ${B.border}`, borderRadius: 10, padding: '22px 24px', marginBottom: 20 }}>
          ...
          {ideasDiff && ideasParsed && (
            <div style={{ marginTop: 16 }}>
              ...
              <button onClick={handleIdeasApply} disabled={ideasSaving || !patSaved}
                style={{ background: (ideasSaving || !patSaved) ? B.surface2 : B.blue, color: (ideasSaving || !patSaved) ? B.dim : '#fff',
                  fontWeight: 700, fontSize: 14, padding: '11px 24px', borderRadius: 7,
                  border: 'none', cursor: (ideasSaving || !patSaved) ? 'default' : 'pointer', marginTop: 4 }}>
                {ideasSaving ? 'Saving...' : `Apply ${ideasParsed.length} Ideas`}
              </button>
            </div>
          )}
        </div>
      ...
      <div style={{ background: B.white, border: `1px solid ${B.border}`, borderRadius: 10, padding: '18px 22px' }}>
        ...
        {[
          ...
          ['4. Apply', 'Click Apply. Changes are saved to the data branch via GitHub. All users see updates within ~5 minutes.'],
        ].map(([step, text]) => (
